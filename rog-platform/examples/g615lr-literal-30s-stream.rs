...
///
/// VARIANT of g615lr-literal-12zone-replay.rs: that test sent all 16
/// packets ONCE then passively waited 15s. Windows session 5 measured real
/// visible-colour latency at ~8-12s AFTER continuous 0x04 streaming
/// begins (not after a one-shot burst) -- this variant continuously
/// re-sends all 16 packets in a loop for 40 seconds instead, to actually
/// test that specific, newly-measured window with real continuous
/// streaming, not a burst-then-wait pattern.
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Windows session 5 measured real visible-colour latency at ~8-12s
    // AFTER 0x04 streaming begins, in a CONTINUOUS streaming test -- every
    // prior Linux test used exactly 8s and/or a one-shot burst + passive
    // wait, not real continuous streaming. This variant continuously
    // re-sends all 16 literal packets in a loop for 40 seconds (comfortable
    // margin past the measured 8-12s window plus its ~4-5s uncertainty),
    // watching for the moment it might resolve mid-stream.
    println!("Continuously streaming all 16 literal packets for 40 seconds...");

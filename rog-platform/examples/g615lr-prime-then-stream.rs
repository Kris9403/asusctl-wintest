...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // The real capture streams 0x04 writes continuously after priming
    // (roughly every 200-800ms, never stopping) -- a single one-shot write
    // (tried previously) never gets a chance to override the RainbowCycle
    // the priming packet visibly triggers. Stream for 8 seconds instead.
    #[rustfmt::skip]
    ...
    println!("Streaming zone 0x06 = red for 8 seconds...");
    ...
    println!("Sent {sent} zone packets.");

...
///
/// VARIANT, the actual new hypothesis (Windows session 5's reframing +
/// Linux session 5's flicker finding): every prior test either triggers
/// RainbowCycle via the priming triplet immediately before `0x04`, or
/// skips it and gets an inert dark baseline -- NEVER "trigger
/// RainbowCycle, then explicitly CANCEL it with a real Static command,
/// THEN attempt 0x04." The flicker finding showed 0x04 writes DO land
/// but get overwritten by RainbowCycle's own still-running animation
/// refresh loop on its very next tick -- so this test explicitly stops
/// that loop first (a real `0x5d` Static command, using the PROVEN
/// correct b3,b5,b4 order for actual colour-setting, not the priming
/// triplet's b3,b4,b5 order) before ever touching `0x04`.
///
/// Sequence: prime into RainbowCycle (same as every other test, to start
/// from the same real-world "already animating" state) -> wait -> cancel
/// with a real Static command -> wait -> THEN stream 0x04 zone writes.
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("Priming sent -- RainbowCycle should be animating now. Waiting 2s for it to actually be running...");
    std::thread::sleep(Duration::from_secs(2));

    // THE NEW STEP: explicitly cancel RainbowCycle with a real Static
    // command, using the PROVEN correct b3,b5,b4 order (not the priming
    // triplet's b3,b4,b5) -- this is the exact sequence a real
    // `asusctl aura effect static -c <hex>` call produces, captured and
    // confirmed working earlier this session. Static mode = 0x00, colour
    // = black (a neutral "off" state, not trying to preempt anything the
    // 0x04 writes will set).
    #[rustfmt::skip]
    let cancel_b3: [u8; 64] = [
    ...
    let mut cancel_b5 = [0u8; 64];
    cancel_b5[0] = 0x5d;
    cancel_b5[1] = 0xb5;
    let mut cancel_b4 = [0u8; 64];
    cancel_b4[0] = 0x5d;
    cancel_b4[1] = 0xb4;
    send!("0x025d b3 (CANCEL -> Static black) iface0", 0x09, 0x025du16, 0u16, &cancel_b3);
    send!("0x025d b5 (set) iface0", 0x09, 0x025du16, 0u16, &cancel_b5);
    send!("0x025d b4 (apply) iface0", 0x09, 0x025du16, 0u16, &cancel_b4);

    println!("Cancel sent -- should be static/dark now, not animating. Waiting 2s to confirm settled...");
    std::thread::sleep(Duration::from_secs(2));
    ...
    // 20s: comfortable margin past the measured 8-12s window, now that
    // RainbowCycle has been explicitly cancelled first (the actual new
    // variable this test isolates -- duration itself was already tested
    // up to 40s in g615lr-literal-30s-stream.rs).
    println!("Continuously streaming all 16 literal packets for 20 seconds...");

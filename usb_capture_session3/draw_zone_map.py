...
the user). Matches the convention already used throughout HANDOFF.md /
aura_core.ps1's $PHYSICAL_MAP.

Each label shows: physical name (what a human says) / internal name
(what aura_core.ps1 calls it) / wire zone ID (what's actually in the 0x04
packet's zone-ID bytes) -- so a report like "zone X lit up" can be matched
straight back to a specific hex byte with no ambiguity.
"""
...
ax.axis("off")
ax.set_title(
    "ROG Strix G16 2025 (G615LR) — Aura zone map\n"
...
def zone(x, y, physical, internal, wire, color="#3b82f6", ha="center", va="center", dx=0, dy=0):
    ax.add_patch(Circle((x, y), 0.16, facecolor=color, edgecolor="black", linewidth=1.2, zorder=5))
    label = f"{physical}\n{internal}  (0x{wire:02X})"
...
# --- Back edge: corners + bar ---
zone(1.9, 8.3, "back_corner_left", "corner_tl", 0x06, LIGHTBAR, ha="right", dx=-0.3, dy=0.35)
zone(3.7, 8.5, "back_left", "back_bar_left", 0x04, LIGHTBAR, dy=0.5)
zone(6.3, 8.5, "back_right", "back_bar_right", 0x05, LIGHTBAR, dy=0.5)
zone(8.1, 8.3, "back_corner_right", "corner_tr", 0x07, LIGHTBAR, ha="left", dx=0.3, dy=0.35)

# --- Front edge: corners + bar ---
zone(1.9, 1.2, "front_corner_left", "corner_bl", 0x0D, LIGHTBAR, ha="right", dx=-0.3, dy=-0.35)
zone(3.7, 1.0, "front_left", "front_bar_left", 0x0F, LIGHTBAR, dy=-0.5)
zone(6.3, 1.0, "front_right", "front_bar_right", 0x0E, LIGHTBAR, dy=-0.5)
zone(8.1, 1.2, "front_corner_right", "corner_br", 0x0C, LIGHTBAR, ha="left", dx=0.3, dy=-0.35)

# --- Left side: front half + back half ---
zone(1.55, 6.3, "left_bar_back", "right_sidebar_back", 0x0B, LIGHTBAR, ha="right", dx=-0.5, dy=0)
zone(1.55, 3.0, "left_bar_front", "left_sidebar_front", 0x09, LIGHTBAR, ha="right", dx=-0.5, dy=0)

# --- Right side: front half + back half ---
zone(8.45, 6.3, "right_bar_back", "left_sidebar_back", 0x08, LIGHTBAR, ha="left", dx=0.5, dy=0)
zone(8.45, 3.0, "right_bar_front", "right_sidebar_front", 0x0A, LIGHTBAR, ha="left", dx=0.5, dy=0)

# --- Keyboard zones, left to right ---
zone(3.15, 6.1, "kbd1", "kbd1", 0x00, KEYBOARD, dy=0.55)
zone(4.35, 6.1, "kbd2", "kbd2", 0x01, KEYBOARD, dy=0.55)
zone(5.65, 6.1, "kbd3", "kbd3", 0x02, KEYBOARD, dy=0.55)
zone(6.85, 6.1, "kbd4", "kbd4", 0x03, KEYBOARD, dy=0.55)

...
$p = "$dir\allcorners.pcapng"

$frames = & "C:\Program Files\Wireshark\tshark.exe" -r $p -Y "frame contains 21:09:04:03" -T fields -e frame.number 2>&1 | Select-Object -First 20
...
foreach ($fr in $frames) {
  ...
  $count = $bytes[1]
  # print raw bytes with index for manual/automated inspection
  $indexed = for ($i=0; $i -lt $bytes.Length; $i++) { "$i=$('{0:X2}' -f $bytes[$i])" }
  Write-Output "frame $fr count=$count : $($indexed -join ' ')"
}

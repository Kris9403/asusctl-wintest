...
# Static mode (mode=0), zone=0 (all), bright magenta -- impossible to miss if it works
$effectPkt = Build-Effect-Packet -zone 0 -mode 0 -r1 255 -g1 0 -b1 255 -speed 0xeb -dir 0 -r2 0 -g2 0 -b2 0
$setPkt = Build-Simple-Packet -cmd 0xb5
$applyPkt = Build-Simple-Packet -cmd 0xb4

Write-Output "Effect packet: $(($effectPkt | ForEach-Object { $_.ToString('X2') }) -join ' ')"

$ok1 = [HidSend]::TrySetOutputReport($targetPath, $effectPkt)
Write-Output "effect (b3, Static/Magenta): $(if ($ok1) {'SUCCESS'} else {"failed err=$([HidSend]::LastError())"})"
$ok2 = [HidSend]::TrySetOutputReport($targetPath, $setPkt)
Write-Output "set (b5): $(if ($ok2) {'SUCCESS'} else {"failed err=$([HidSend]::LastError())"})"
$ok3 = [HidSend]::TrySetOutputReport($targetPath, $applyPkt)
